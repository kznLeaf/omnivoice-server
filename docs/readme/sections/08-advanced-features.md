...
response = httpx.post(
    ...
    json={"input": "Hello [laughter] this is amazing [breath] really cool [sigh]"},
)
```
...
```python
response = httpx.post(
    "http://127.0.0.1:8880/v1/audio/speech", json={"input": "这是拼音(pīn yīn)提示的例子"}
)
```
...
response = httpx.post(
    "http://127.0.0.1:8880/v1/audio/speech",
    json={"input": "The word read(R IY D) is pronounced differently from read(R EH D)"},
)
```
